defmodule FogetesTest.Servicos.AlunoServicoTest do
  ...
  test "Criar instancia através de um map" do
    # técnica AAA
    ...
    # Act (ação / execução)
    aluno = AlunoServico.instancia(map)

    # Assert (Validação)
    assert is_struct(aluno, Aluno)
    assert aluno.id == 1
    assert aluno.nome == "danilo"
    assert aluno.matricula == "2"
    assert length(aluno.notas) == 3
  end
  ...
  test "Incluir aluno com nome inválido" do
    # técnica AAA
    ...
    # Act (ação / execução)
    # Assert (Validação)
    assert_raise RuntimeError, "Nome é obrigatório", fn ->
      AlunoServico.incluir(alunos, aluno)
    end
  end

  test "Incluir aluno com matricula inválido" do
    # técnica AAA
    ...
    # Act (ação / execução)
    # Assert (Validação)
    assert_raise RuntimeError, "Matrícula é obrigatório", fn ->
      AlunoServico.incluir(alunos, aluno)
    end
  end

  test "Incluir aluno com notas inválida" do
    # técnica AAA
    ...
    # Act (ação / execução)
    # Assert (Validação)
    assert_raise RuntimeError, "O aluno precisa ter pelo menos uma nota", fn ->
      AlunoServico.incluir(alunos, aluno)
    end
  end
