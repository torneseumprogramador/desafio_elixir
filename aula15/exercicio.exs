...
defmodule Classe do
  ...
  def alterar_aluno(alunos) do
    ...
    if aluno == nil do
      ...
      # aluno = Map.merge(aluno, aluno_a_alterar)
      # alunos = alunos ++ [aluno]

      ### Forma 2
      alunos = Enum.map(alunos, fn
        %{matricula: ^matricula} = aluno_do_map -> Map.merge(aluno_do_map, aluno_a_alterar)
        aluno_do_map -> aluno_do_map
      end)
